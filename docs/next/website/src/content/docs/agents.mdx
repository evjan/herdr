...
Bundled manifests live inside Herdr. A local override in the config directory replaces the bundled manifest for that agent.
...
The running server loads active manifests into memory on startup. After editing a local override, restart Herdr or run `herdr server reload-agent-manifests` to apply the file to the running server.
...
Install the integration for each agent you use to give Herdr hook reports instead of screen detection alone:
