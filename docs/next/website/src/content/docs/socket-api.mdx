...
- report custom agent state from hooks
...
Pane graphics let an API caller place image data over a pane. They are available by
...
filter reported pane metadata. Values are strings, booleans, unsigned
...
`source` identifies the owner. Callers choose their own source string. A
successful set atomically replaces the
previous view. The view lasts until it is cleared, replaced, its owner
...
{"id":"view_clear_owned","method":"agent.view.clear","params":{"source":"example.agent-views"}}
```
...
Workspace get/list responses expose the resulting `tokens` map, and Space sidebar rows can render values such as `$jj_status`. Changes and TTL expiry emit `workspace.metadata_updated` with the latest workspace snapshot. This metadata event is available to API subscribers.
...
Fields such as `active_version` and `warning` are omitted when not available. `server.reload_agent_manifests` returns `agent_manifest_reload` with the same `manifests` item shape after reloading the in-memory rule cache.
