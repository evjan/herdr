...
/// An endpoint shell that has no surface yet waits on the server's first full
/// projection, which can take far longer than a plain attach handshake.
pub(super) const ENDPOINT_HANDSHAKE_READ_TIMEOUT: Duration = Duration::from_secs(60);
...
pub(super) fn do_handshake(
    ...
    let read_timeout = if endpoint_shell && !surface_active {
        ENDPOINT_HANDSHAKE_READ_TIMEOUT
    } else {
